...
contract LitedexSP is BEP20('Litedex Stake Pool', 'LDX-SP') {
    ...
    function safeLdxTransfer(address _to, uint256 _amount) public onlyOwner {
        ...
        if (_amount > ldxBalance) {
            ...
            require(ldx.transfer(_to, ldxBalance), "Invalid Transfer");
        } else {
...
contract LitedexFarmer is Ownable {
    ...
    LitedexSP private sldx;
    
    /* LDX tokens created per block. */
    uint256 private ldxPerBlock;
    
    /* Bonus muliplier for early LDX makers. */
    uint256 private BONUS_MULTIPLIER = 1;
    ...
    /* view ldx minted per block*/
    function totalLdxPerBlock() external view returns(uint256){
        return ldxPerBlock;
    }
    ...
    function emergencyWithdraw(uint256 _pid) external validatePoolByPid(_pid) {
        ...
        UserInfo storage user = userInfo[_pid][msg.sender];
        if(_pid == 0){
            sldx.burn(msg.sender, user.amount);
        }
